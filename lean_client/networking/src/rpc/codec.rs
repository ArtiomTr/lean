...
//     fn phase0_block() -> SignedBeaconBlock {
//         factory::full_phase0_signed_beacon_block().into()
//     }
//     fn altair_block(config: &Config) -> SignedBeaconBlock {
...
//     fn empty_base_block() -> SignedBeaconBlock {
//         factory::empty_phase0_signed_beacon_block().into()
//     }

//     fn empty_blob_sidecar(config: &Config) -> Arc<BlobSidecar> {
//         // The context bytes are now derived from the block epoch, so we need to have the slot set
//         // here.
//         let mut blob_sidecar = BlobSidecar::default();
//         blob_sidecar.signed_block_header.message.slot =
//             misc::compute_start_slot_at_epoch(config.deneb_fork_epoch);
//         Arc::new(blob_sidecar)
//     }

//     fn empty_fulu_data_column_sidecar(config: &Config) -> Arc<DataColumnSidecar> {
//         // The context bytes are now derived from the block epoch, so we need to have the slot set
//         // here.
//         let mut data_column_sidecar = FuluDataColumnSidecar::default();
//         data_column_sidecar.signed_block_header.message.slot =
//             misc::compute_start_slot_at_epoch(config.fulu_fork_epoch);
//         Arc::new(data_column_sidecar.into())
//     }

//     fn empty_gloas_data_column_sidecar(config: &Config) -> Arc<DataColumnSidecar> {
//         let mut data_column_sidecar = GloasDataColumnSidecar::default();
//         data_column_sidecar.slot = misc::compute_start_slot_at_epoch(config.gloas_fork_epoch);
//         Arc::new(data_column_sidecar.into())
//     }

//     /// Bellatrix block with length < max_rpc_size.
//     fn bellatrix_block_small(config: &Config) -> BellatrixSignedBeaconBlock {
//         // The context bytes are now derived from the block epoch, so we need to have the slot set
//         // here.
//         let tx = ByteList::from_ssz_default([0; 1024]).unwrap();
...
//                 },
//                 slot: misc::compute_start_slot_at_epoch(config.bellatrix_fork_epoch),
...
//     fn bellatrix_block_large(config: &Config) -> BellatrixSignedBeaconBlock {
//         // The context bytes are now derived from the block epoch, so we need to have the slot set
//         // here.
//         let tx = ByteList::from_ssz_default([0; 1024]).unwrap();
...
//     fn dcbroot_request(config: &Config) -> DataColumnsByRootRequest {
...
//         let fork_context = Arc::new(ForkContext::dummy(config, fork_name));

//         let mut buf = BytesMut::new();
//         let mut snappy_inbound_codec = SSZSnappyInboundCodec::new(
//             config.clone_arc(),
//             snappy_protocol_id,
...
//         fork_name: Phase,
//     ) -> Result<BytesMut, RPCError> {
//         let fork_context = ForkContext::dummy(config, fork_name);
...
//         let mut inbound_codec = SSZSnappyInboundCodec::new(
//             config.clone_arc(),
//             protocol.clone(),
//             config.max_payload_size,
//             fork_context.clone(),
...
//         let bellatrix_block_small = bellatrix_block_small(&config);
//         let bellatrix_block_large = bellatrix_block_large(&config);
...
//         let config = Arc::new(Config::mainnet().rapid_upgrade());
//         let fork_context = ForkContext::dummy(&config, Phase::Altair);
...
//             RequestType::DataColumnsByRoot(dcbroot_request(&config)),
...
//         };
//         for fork_name in enum_iterator::all::<Phase>() {
//             let requests: [RequestType; 2] = fork_dependent_requests(fork_name);
//             for req in requests {
//                 encode_then_decode_request(&config, req.clone(), fork_name);
//             }
//         }
...
//         let config = Arc::new(Config::mainnet().rapid_upgrade());
//         let fork_context = Arc::new(ForkContext::dummy(&config, Phase::Altair));
...
//         let block_message_bytes = altair_block(&config).to_ssz().unwrap();
...
//         let fork_context = Arc::new(ForkContext::dummy(&config, Phase::Phase0));
